...
type Row = PosPerformanceRow & { id: string; aroName: string };
...
function BdoPosPerformanceContent() {
  ...
  const agentAroLookup = useMemo(() => new Map(agents.map((a) => [a.id, a.aroId])), []);
  const aroNameLookup = useMemo(() => new Map(aros.map((a) => [a.id, a.name])), []);

  const rows = useMemo<Row[]>(() => {
    return getPosPerformanceRows(scopeAroId, {
      ...
      .map((r) => ({
        ...r,
        id: r.terminalId,
        aroName: aroNameLookup.get(agentAroLookup.get(r.agentId) ?? "") ?? "—",
      }));
  }, [scopeAroId, resolvedRange, values, agentAroLookup, aroNameLookup]);
  ...
  const columns: Column<Row>[] = [
    { header: "Terminal", render: (r) => <span className="font-semibold text-ink-900">{r.serial}</span> },
    { header: "Status", render: (r) => <Badge tone={statusTone(r.status)}>{r.status}</Badge> },
    {
      ...
      render: (r) => (
        <div>
          ...
          <p className="text-xs text-ink-400">{r.aroName}</p>
        </div>
      ),
    },
    {
  ...
  return (
    <div>
      <PageHeader title="POS Performance" description="Every POS terminal across the network." />
        ...
        <Card className="p-5">
          ...
          {best ? (
            <div>
              ...
              <p className="text-xs text-ink-400">{best.agentName}</p>
        ...
        <Card className="p-5">
          ...
          {least ? (
            <div>
              ...
              <p className="text-xs text-ink-400">{least.agentName}</p>
      ...
      <Card>
        ...
        <div className="mt-4">
          <Table
            ...
            rows={rows}
            pageSize={10}
            onRowClick={(r) => router.push(`/bdo/agents/${r.agentId}?tab=transactions&posTerminalId=${r.terminalId}`)}
...
export default function BdoPosPerformancePage() {
  const allowed = useRequireAccess("bdo");
  if (!allowed) return null;
  return (
    <Suspense fallback={null}>
      <BdoPosPerformanceContent />
    </Suspense>
  );
}
