...
export function Sidebar() {
  ...
  const { userType, sidebarCollapsed, toggleSidebar, mobileNavOpen, closeMobileNav } = useApp();
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          "flex h-screen flex-col bg-navy-950 text-navy-50 transition-transform duration-200",
          // Mobile: fixed off-canvas drawer, always full width, slides in/out
          "fixed inset-y-0 left-0 z-50 w-[264px]",
          mobileNavOpen ? "translate-x-0" : "-translate-x-full",
          // Desktop: sticky in-flow, always visible, width toggles via collapse state
      ...
      >
        ...
        <nav className="scrollbar-thin flex-1 overflow-y-auto px-3 py-4">
          {sections.map((section, sIdx) => (
            <div key={sIdx} className="mb-4">
              {section.heading && (!sidebarCollapsed || mobileNavOpen) && (
        ...
        {/* Collapse toggle — desktop only, no-op concept on mobile drawer */}
        <button
          onClick={toggleSidebar}
          className="hidden items-center gap-2 border-t border-white/5 px-5 py-4 text-xs font-medium text-navy-300 hover:text-white lg:flex"
        >
